...
const search = ({
  ...
  new Promise((resolve, reject) => {
    axios
      .get(
        `https:\/\/cors-anywhere.herokuapp.com\/https:\/\/${city}${baseSearchUrl}${category ||
          ''}`,
        {
          params: {
            query,
      ...
      .then(({ data }) => {
        ...
        const pTagArray = []
        let matchArray
        while ((matchArray = resultPTagRegex.exec(formatedData)))
          pTagArray.push(matchArray[1])
  ...
  new Promise((resolve, reject) => {
    ...
    axios
      ...
      .then(({ data }) => {
        ...
        let imageMatch
        while ((imageMatch = regexDetailImages.exec(matchResult[4])))
          imagesArray.push(imageMatch[1])
